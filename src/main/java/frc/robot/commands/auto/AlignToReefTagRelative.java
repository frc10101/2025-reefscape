...
public class AlignToReefTagRelative extends Command {
  ...
  public void initialize() {
    ...
    yController.setSetpoint(
        isRightScore
            ? AlignToReefTagRelative_Constants.Y_SETPOINT_REEF_ALIGNMENT
            : -AlignToReefTagRelative_Constants.Y_SETPOINT_REEF_ALIGNMENT);
  ...
  public void execute() {
    if (LimelightHelpers.getTV("")
        && LimelightHelpers.getFiducialID(LimelightConstants.limelightName) == tagID) {
      this.dontSeeTagTimer.reset();

      double[] postions = LimelightHelpers.getBotPose_TargetSpace(LimelightConstants.limelightName);
      ...
      drivebase.drivePlease(new Translation2d(xSpeed, ySpeed), rotValue, false);

      if (!rotController.atSetpoint() || !yController.atSetpoint() || !xController.atSetpoint()) {
        stopTimer.reset();
      }
    } else {
      drivebase.drivePlease(new Translation2d(), 0, false);
    }
